...
The main Arduino file is RFID_Excel.ino
.
This code performs the following:

Initializes the RFID RC522 module.

Reads unique RFID tag IDs when a tag/card is placed near the reader.

Sends the data over Serial communication to the connected PC.

The data can be captured into an Excel sheet or any logging software.

⚙️ How to Run

Open RFID_Excel.ino in the Arduino IDE.

Select the Arduino UNO board and correct COM port.

Upload the code to the Arduino.

Open the Serial Monitor (9600 baud rate).

Place an RFID card near the reader to see its unique ID appear.

You can log the IDs in Excel using the “Data → From Serial Port” option or via Python/C# script.
